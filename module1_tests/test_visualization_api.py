...
class VisualizationApiTests(unittest.TestCase):
    ...
    def test_tc013_historical_images_display_on_windows(self):
        if os.environ.get("SKYGUARD_TEST_PHASE", "discovery") == "discovery":
            legacy_url = "/visualization/image/task-1/comparison\\a.jpg"
            self.assertNotIn(
                "\\",
                legacy_url,
                "SG-002：历史结果图片 URL 含 Windows 反斜杠，浏览器无法正常加载",
            )
